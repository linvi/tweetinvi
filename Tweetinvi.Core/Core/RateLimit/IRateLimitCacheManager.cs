...
namespace Tweetinvi.Core.RateLimit
{
    ...
    public interface IRateLimitCacheManager
    {
        ...
        /// </summary>
        Task<IEndpointRateLimit> GetOrCreateQueryRateLimit(string query, ITwitterCredentials credentials);

        /// <summary>
        /// Return the rate limits for a specific query. 
        /// If the rate limits are not located in the cache, they will be retrieved from Twitter.
        /// </summary>
        Task<IEndpointRateLimit> GetQueryRateLimit(string query, ITwitterCredentials credentials);

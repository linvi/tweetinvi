...
namespace Tweetinvi.Core.Helpers
{
    ...
    {
        string HtmlEncode(string unicodeText);
        string HtmlEncode(string unicodeText, bool encodeTagsToo);

        string HtmlDecode(string unicodeText);
        string HtmlDecode(string unicodeText, bool encodeTagsToo);
    }

    public class HttpUtility : IHttpUtility
    {
        private readonly Regex _entityResolver = new Regex("([&][#](?'decimal'[0-9]+);)|([&][#][(x|X)](?'hex'[0-9a-fA-F]+);)|([&](?'html'\\w+);)");

        public string HtmlEncode(string unicodeText)
        {
            return HtmlEncode(unicodeText, true);
        }

        public string HtmlEncode(string unicodeText, bool encodeTagsToo)
        {

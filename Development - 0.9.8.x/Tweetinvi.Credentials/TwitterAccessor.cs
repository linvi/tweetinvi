...
namespace Tweetinvi.Credentials
{
    public class TwitterAccessor : ITwitterAccessor
    {
        ...
        public string ExecutePOSTMultipartQuery(string query, IEnumerable<IMedia> medias, string contentId)
        {
            return ExecuteMultipartQuery(query, HttpMethod.POST, medias, contentId);
        }

        public T ExecutePOSTMultipartQuery<T>(string query, IEnumerable<IMedia> medias, string contentId, JsonConverter[] converters = null) where T : class
        {
            string jsonResponse = ExecuteMultipartQuery(query, HttpMethod.POST, medias, contentId);
            return _jsonObjectConverter.DeserializeObject<T>(jsonResponse, converters);
        }
        ...
        public string ExecuteMultipartQuery(string query, HttpMethod method, IEnumerable<IMedia> medias, string contentId)
        {
            if (query == null)
            {
                throw new ArgumentException("At least one of the arguments provided to the query was invalid.");
            ...
            try
            {
                return _twitterRequestHandler.ExecuteMultipartQuery(query, contentId, method, medias);
            }
            catch (TwitterException ex)
            {
                HandleQueryException(ex);
